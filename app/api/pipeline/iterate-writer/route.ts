...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('[Pipeline] Triggering iterate writer job for project:', projectId);
    try {
      const run = await tasks.trigger(
        "iterate-writer-pipeline",
        {
          projectId,
        ...
        { idempotencyKey: projectId + '_iterate_writer_' + Date.now() }
      );

      return NextResponse.json(
        {
          message: 'Iterating writer',
          projectId,
          runId: run.id
        },
        { status: 202 }
    ...
    return NextResponse.json(
      { error: 'Failed to iterate writer' },
      { status: 500 }
    );
  }
}
