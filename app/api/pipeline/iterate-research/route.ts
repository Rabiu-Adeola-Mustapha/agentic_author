...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('[Pipeline] Triggering iterate research job for project:', projectId);
    try {
      const run = await tasks.trigger(
        "iterate-research-pipeline",
        {
          projectId,
        ...
        { idempotencyKey: projectId + '_iterate_research_' + Date.now() }
      );

      return NextResponse.json(
        {
          message: 'Iterating research',
          projectId,
          runId: run.id
        },
        { status: 202 }
    ...
    return NextResponse.json(
      { error: 'Failed to iterate research' },
      { status: 500 }
    );
  }
}
